...
void menu2() {
  ...
  if (m2 == 1) {
    ...
    lcd.print(' ');
  } lcd.print("I="); Ki = (double)KiSet / 1000.0;
  lcd.print(Ki, 3);
  lcd.setCursor(0, 1);
  if (m2 == 2) {
    lcd.write(ar);
...
//ФУНКЦИЯ ФОРМИРОВАНИЯ БОЛЬШИХ ЦИФР ИЗ СИМВОЛОВ ПСЕВДОГРАФИКИ
void bigdigit() {
...
//ФОРМИРУЕМ СИМВОЛЫ ПСЕВДОГРАФИКИ ДЛЯ БОЛЬШИХ ЦИФР В ГЛАВНОМ МЕНЮ
void bigdigitres() {
...
//ПЕРЕЗАПИСЫВАЕМ СИМВОЛЫ ПСЕВДОГРАФИКИ ДЛЯ БОЛЬШИХ ЦИФР В ГЛАВНОМ МЕНЮ
void arrow0() {
  byte v7[8] = { 0, 0, 0, 0, 0, 0, 7, 7};
  byte v8[8] = {31, 31, 0, 0, 0, 0, 0, 0};
  lcd.createChar(7, v7); lcd.createChar(8, v8);
...
//ФУНКЦИЯ ФОРМИРОВАНИЯ СТРЕЛОК-УКАЗАТЕЛЕЙ ДЛЯ МЕНЮ
void arrow1() {
