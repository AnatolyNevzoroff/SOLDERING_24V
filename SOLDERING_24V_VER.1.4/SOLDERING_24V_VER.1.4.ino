...
#define p400 400L
#define p100 100L
//ИСКУССТВЕННАЯ ПАЗУЗА ДЛЯ ИНЖЕНЕРНОГО МЕНЮ. ЕСЛИ ТЭН ПЕРЕГРЕВАЕТСЯ - ЗНАЧЕНИЕ УВИЛИЧИТЬ ДО \
29L ИЛИ 30L ПРИ НЕДОСТАТОЧНОЙ МОЩНОСТИ ТЭН - УМЕНЬШИТЬ ДО 27L ИЛИ 26L
#define pause 28L
...
int16_t testtemp;
int16_t freq[4] = {1370, 1170, 970, 770}; //МАССИВ ДЛЯ ФУНКЦИИ tone()
...
void clmil(); void clful(); int16_t median(uint8_t samples); void bigtime(); void get_time();
void set_time();
...
void setup() {
  ...
  ///////////////////////////////////// В Н И М А Н И Е !!! ////////////////////////////////////////
  //ВО ИЗБЕЖАНИЕ "СТРАННОГО" ПОВЕДЕНИЯ ПАЯЛЬНИКА ПРИ САМОЙ ПЕРВОЙ ЗАГРУЗКИ СКЕТЧА: \
...
void loop() {
  ...
  if (power == true) {
    ...
    ///////////////////////////////////////// ЯДРО ПАЯЛЬНИКА /////////////////////////////////////////
    inputdata = median(15); //ЧИТАЕМ ОТФИЛЬТРОВАННЫЕ СРЕДНЕ-МЕДИАННЫЕ (1 ИЗ 15) ЗНАЧЕНИЯ С ПОРТА "А0"
    ...
    if (menu == 0) {
      ...
      ВКЛЮЧАЕМ ИНДИКАТОР "POWER", ПОДАЁМ ЗВУКОВОЙ СИГНАЛ ЕСЛИ ВЫВОДИМ ПАЯЛЬНИК ИЗ СНА ОТ ДАТЧИКА
      if (f.w1 == true) {
      ...
      lcd.setCursor(0, 0); graf = Input; digprint3(); lcd.write(B11011111);
        //ОТОБРАЖАЕМ СКВАЖНОСТЬ ШИМ В ПРОЦЕНТАХ (ДЛЯ АНТУРАЖА)
        pwm = map(Output, 0, 255, 0, 100); lcd.setCursor(0, 1); graf = pwm; digprint3(); lcd.print("% ");
        //ОТОБРАЖАЕМ ТЕМПЕРАТУРУ ВНУТРИ КОРПУСА
    ...
    2. I = - КОЭФФИЦИЕНТ ИНТЕГРИРОВАНИЯ. ДИАПАЗОН ЗНАЧЕНИЙ 0.00 - 9.99 (РЕКОМЕНДОВАНО 0.02) \
    ...
    if (menu == 2) {
      ...
      if (f.w6 == false && newPos != oldPos) {
        ...
        switch (m2) {
          ...
          case 1: KiSet = constrain(trig2, 0, 999); break;
  ...
  if (power == false) {
    ...
    //ПОДАЁМ ЗВУКОВОЙ СИГНАЛ ОТКЛЮЧЕНИЯ ПАЯЛЬНИКА (БЛОКА ПИТАНИЯ 24В)
    ...
    //ПЕРЕХОД В ТЕХНИЧЕСКОЕ МЕНЮ НАСТРОЙКИ
    ...
    if (menu == 0) {
      ...
      if (f.w5 == true) {
        ...
        analogWrite(10, 0); digitalWrite(13, HIGH); digitalWrite(12, LOW);
      }
      ...
      if (f.w8 == true && mill > timer2 + p1000) {
        ...
        temp = map(inputdata, 0, maxADC, 20, 350);
        ...
        //ЕСЛИ ПАЯЛЬНИК ОСТЫЛ, УПРАВЛЯЕМ ПОДСВЕТКОЙ ЭКРАНА, ПОДАЁМ ЗВУКОВОЙ СИГНАЛ И ОТКЛЮЧАЕМ ИНДИКАТОР
          ...
          (led ? ledON : ledOFF); f.w7 = false; f.w8 = false; tone(11, 1300, 350);
      ...
      if (f.w8 == false && mill - timer3 > p1000) {
        ...
        temp = map(inputdata, 0, maxADC, 20, 350); bigtime(); lcd.setCursor(13, 0); lcd.print('+'); lcd.print(temp);
      }
    }//END MENU 0
    ...
    if (menu == 1) {
      ...
      if (f.w5 == true) {
        f.w5 = false; f.w6 = true; setime = 0; clmil(); get_time(); ledON;
    ...
    if (menu == 2) {
      ...
      МЕНЯЯ СКВАЖНОСТЬ ШИМ (В ОЧЕНЬ УЗКОМ ДИАПАЗОНЕ ТЕМПЕРАТУР!)
      if (newPos != oldPos) {
      ...
      //РЕГУЛИРУЯ ТЕМПЕРАТУРУ ТОЛЬКО СКВАЖНОСТЬЮ ШИМ НЕВОЗМОЖНО БАЛАНСИРОВАТЬ НА ОПРЕДЕЛЁННОЙ \
      ТЕМПЕРАТУРЕ, ИЗМЕНЕНИЕ СКВАЖНОСТИ В НАЧАЛЕ ШКАЛЫ ПРИВОДИТ К ОЧЕНЬ РЕЗКИМ ПЕРЕПАДАМ ТЕМПЕРАТУРЫ \
      НАПРИМЕР ПРИ СКВАЖНОСТИ 5 ТЕМПЕРАТУРА НЕ БУДЕТ СУЩЕСТВЕННО РАСТИ И СПУСТЯ ДЛИТЕЛЬНОЕ ВРЕМЯ \
      НЕ ПРЕВЫСИТ 50 ГРАДУСОВ, А ПРИ СКВАЖНОСТИ 6 БУДЕТ НЕПРЕРЫВНО РАСТИ И ПРЕВЫСИТ 350 \
      (ДАЛЬШЕ Я НЕ ПРОБОВАЛ, МОЖЕТ ДО ПОЛНОГО ВЫГОРАНИЯ ТЭН, ПОЭТОМУ ВВОДИМ ИСКУСТВЕННУЮ ПАУЗУ, \
      ТАК КАК ДРОБНОЕ ЗНАЧЕНИЕ ШИМ НЕВОЗМОЖНО)

      //НАГРЕВАЕМ ТЭН ПОДАВАЯ ШИМ ЧЕРЕЗ ПАУЗУ (БЕЗ ИСПОЛЬЗОВАНИЯ БИБЛИОТЕКИ ПИД РЕГУЛИРОВАНИЯ)
      if (mill - timer5 > (f.w6 ? 1 : pause)) {
